...
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'packandgo-secret-key-2024')
...
def send_email_brevo(to_email: str, subject: str, body_text: str = "", body_html: str = ""):
    ...
    payload = {
        ...
        "htmlContent": body_html if body_html else f"<pre>{body_text}</pre>",
    }
    ...
    if resp.status_code not in (200, 201):
        raise Exception(f"Brevo HTTP {resp.status_code}: {resp.text}")

    print(f"✅ Email Brevo envoyé à {to_email} — messageId: {resp.json().get('messageId')}")
    return resp.json()
...
def get_db_connection():
    return pymysql.connect(
        ...
        database=os.getenv("MYSQDATABASE", "railway"),
...
def login():
    ...
    try:
        ...
        if stored_pw.startswith("SOCIAL_"):
            return jsonify({"message": "Ce compte utilise une connexion sociale. Veuillez vous connecter via le bouton correspondant."}), 401
...
def reset_password():
    ...
    try:
        ...
        send_email_brevo(
            to_email=email,
            subject="Code de vérification — Pack & Go",
            body_text=(
                f"Bonjour,\n\n"
...
def send_invitations():
    ...
    try:
        ...
        invite_code = existing["invite_code"] if existing else generate_invite_code()
...
⚠️  Ce code expire après 24H

Téléchargez l'application, créez un compte et entrez ce code pour rejoindre le voyage.
...
<div style="font-family:Arial,sans-serif;max-width:540px;margin:auto;border:1px solid #D6E4FF;border-radius:16px;overflow:hidden;">
  <div style="background:linear-gradient(135deg,#042A66,#0A4DBF);padding:32px;text-align:center;">
    ...
    <p style="color:rgba(255,255,255,0.75);margin:10px 0 0;">Invitation à un voyage</p>
  </div>
  <div style="padding:32px;">
    <p style="color:#374151;font-size:15px;">Bonjour 👋</p>
    <p style="color:#374151;font-size:15px;">Vous avez été invité(e) à rejoindre un voyage !</p>
    <div style="background:#EEF4FF;border-radius:12px;padding:16px;margin:20px 0;">
      <table style="width:100%;border-collapse:collapse;font-size:13px;">
        <tr><td style="color:#6B7280;padding:4px 0;">🗺️ Destination</td><td style="font-weight:700;text-align:right;color:#042A66;">{destination}</td></tr>
        <tr><td style="color:#6B7280;padding:4px 0;">📅 Départ</td><td style="font-weight:700;text-align:right;color:#042A66;">{date_depart}</td></tr>
        <tr><td style="color:#6B7280;padding:4px 0;">📅 Retour</td><td style="font-weight:700;text-align:right;color:#042A66;">{date_arrivee}</td></tr>
        <tr><td style="color:#6B7280;padding:4px 0;">🌙 Durée</td><td style="font-weight:700;text-align:right;color:#042A66;">{nuitees} nuit(s)</td></tr>
      </table>
    </div>
    <div style="background:linear-gradient(135deg,#C89B3C,#E8B84B);border-radius:14px;padding:24px;text-align:center;margin:24px 0;">
      <p style="color:rgba(61,34,0,0.65);font-size:11px;margin:0 0 6px;letter-spacing:2px;text-transform:uppercase;">Code d'invitation</p>
      <p style="color:#3D2200;font-size:34px;font-weight:900;margin:0;letter-spacing:8px;font-family:monospace;">{invite_code}</p>
      <p style="color:rgba(61,34,0,0.65);font-size:11px;margin:8px 0 0;">⚠️ Expire après 24H</p>
    </div>
    <p style="color:#9CA3AF;font-size:11px;text-align:center;margin-top:20px;">
      Ce message a été envoyé automatiquement par Pack&amp;Go
    </p>
  </div>
</div>
                """.strip()
...
def check_already_joined():
    ...
    try:
        ...
        cursor = conn.cursor()
        if user_id:
            cursor.execute("SELECT id FROM group_preferences WHERE invite_code = %s AND user_id = %s LIMIT 1", (code, user_id))
            row = cursor.fetchone()
            if not row and email:
                cursor.execute("SELECT id FROM group_preferences WHERE invite_code = %s AND LOWER(email) = %s LIMIT 1", (code, email))
                row = cursor.fetchone()
        else:
            cursor.execute("SELECT id FROM group_preferences WHERE invite_code = %s AND LOWER(email) = %s LIMIT 1", (code, email))
            row = cursor.fetchone()
...
def join_with_code():
    ...
    try:
        ...
        if already:
            cursor.close(); conn.close()
            return jsonify({"error": "Déjà rejoint", "message": "Vous avez déjà soumis vos préférences pour ce voyage."}), 409
        cursor.execute("UPDATE invitations SET statut = 'accepte' WHERE invite_code = %s AND LOWER(email_invite) = %s", (code, email))
        conn.commit(); cursor.close(); conn.close()
        return jsonify({
...
def check_overlap():
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE LOWER(gp.email) = %s AND gp.invite_code != %s
        """, (email, invite_code))
        ...
        return jsonify({"overlap": blocked, "blocked": blocked, "min_gap": MIN_GAP_DAYS, "conflicts": conflicts}), 200
    except Exception as e:
        return jsonify({"overlap": True, "blocked": True, "error": str(e), "conflicts": [], "min_gap": MIN_GAP_DAYS}), 500
    finally:
...
def get_pending_invites():
    ...
    try:
        conn   = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT email_invite AS email, statut FROM invitations WHERE invite_code = %s AND statut = 'en_attente'", (code,))
        rows = cursor.fetchall()
        cursor.close(); conn.close()
        return jsonify({"pending": [{"email": r["email"], "statut": r["statut"]} for r in rows]}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def delete_guest():
    ...
    email       = (data.get('email')       or '').strip().lower()
    if not invite_code: return jsonify({'error': 'invite_code manquant'}), 400
    if not email:       return jsonify({'error': 'email manquant'}), 400
    try:
        conn   = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM group_preferences WHERE invite_code = %s AND LOWER(email) = %s", (invite_code, email))
        if cursor.rowcount == 0:
            conn.rollback(); cursor.close(); conn.close()
            return jsonify({'error': f"Aucun invité trouvé avec email={email!r}"}), 404
        cursor.execute("UPDATE invitations SET statut = 'en_attente' WHERE invite_code = %s AND LOWER(email_invite) = %s", (invite_code, email))
        conn.commit(); cursor.close(); conn.close()
...
def update_invite_email():
    ...
    try:
        conn   = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM invitations WHERE invite_code = %s AND email_invite = %s AND statut = 'en_attente' LIMIT 1", (code, old_email))
        if not cursor.fetchone():
            cursor.close(); conn.close()
            return jsonify({"error": "Invité introuvable ou a déjà répondu"}), 404
        cursor.execute("SELECT id FROM invitations WHERE invite_code = %s AND email_invite = %s LIMIT 1", (code, new_email))
        if cursor.fetchone():
            cursor.close(); conn.close()
            return jsonify({"error": "Ce nouvel email est déjà invité dans ce voyage"}), 409
        cursor.execute("UPDATE invitations SET email_invite = %s WHERE invite_code = %s AND email_invite = %s", (new_email, code, old_email))
        conn.commit(); cursor.close(); conn.close()
...
def notify_leader():
    ...
    errors = []
    try:
        action_label = "✅ A rejoint le voyage" if action == "rejoint" else "✏️ A mis à jour ses informations"
        subject      = f"✈️ Un invité a rejoint votre voyage vers {destination}" if action == "rejoint" else f"✏️ Un invité a modifié ses informations — {destination}"
        html_body = f"""
        <div style="font-family:Arial,sans-serif;max-width:540px;margin:auto;border:1px solid #D6E4FF;border-radius:16px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#042A66,#0A4DBF);padding:32px;text-align:center;">
            <h1 style="color:#fff;margin:0;font-size:24px;">✈️ Pack&amp;Go</h1>
          ...
          <div style="padding:32px;">
            ...
            <p style="color:#4B5563;font-size:15px;"><strong>{guest_email}</strong> — {action_label}</p>
            <div style="background:#EEF4FF;border-radius:12px;padding:16px;margin:20px 0;">
              <table style="width:100%;border-collapse:collapse;font-size:13px;">
        ...
        send_email_brevo(to_email=leader_email, subject=subject, body_html=html_body)
    except Exception as e:
    ...
    if not errors:        return jsonify({"status": "ok"}), 200
    elif len(errors) < 2: return jsonify({"status": "partial", "errors": errors}), 207
...
def update_invite_info():
    ...
    try:
        ...
        if not cursor.fetchone():
            cursor.close(); conn.close()
            return jsonify({"error": "Code invalide ou expiré"}), 404
        cursor.execute("UPDATE invitations SET destination=%s, date_depart=%s, date_arrivee=%s, nuitees=%s WHERE invite_code=%s",
...
def get_group_preferences():
    ...
    try:
        conn   = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM group_preferences WHERE invite_code = %s", (invite_code,))
        rows = cursor.fetchall()
        cursor.close(); conn.close()
...
def update_group_preferences():
    ...
    try:
        conn   = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM group_preferences WHERE invite_code = %s AND LOWER(email) = %s LIMIT 1", (invite_code, guest_email))
        existing = cursor.fetchone()
        if existing:
            cursor.execute("""
                UPDATE group_preferences
                SET destination=%s, date_depart=%s, date_arrivee=%s, nuitees=%s
                WHERE invite_code=%s AND LOWER(email)=%s
            """, (destination, date_depart, date_arrivee, nuitees, invite_code, guest_email))
            conn.commit(); cursor.close(); conn.close()
            return jsonify({"status": "updated", "message": "Informations voyage mises à jour"}), 200
        else:
            cursor.close(); conn.close()
            return jsonify({"status": "not_found", "message": "Aucune préférence trouvée pour cet invité"}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def save_group_preferences():
    ...
    if not invite_code: return jsonify({"error": "invite_code requis"}), 400
    if not user_id:     return jsonify({"error": "user_id requis"}), 400
    try:
        conn   = get_db_connection()
        cursor = conn.cursor()
        ...
        if role == 'leader':
            cursor.execute("""
                SELECT gp.user_id FROM group_preferences gp
                INNER JOIN users u ON u.id = gp.user_id
                WHERE gp.invite_code = %s AND gp.role = 'leader' LIMIT 1
            """, (invite_code,))
            existing_leader = cursor.fetchone()
            if existing_leader and existing_leader['user_id'] != user_id:
                cursor.close(); conn.close()
                return jsonify({"error": "Ce voyage a déjà un leader.", "message": "Un autre utilisateur est déjà leader."}), 403

        cursor.execute("SELECT destination, date_depart, date_arrivee, nuitees FROM invitations WHERE invite_code = %s LIMIT 1", (invite_code,))
        inv = cursor.fetchone()
        if not inv:
            cursor.close(); conn.close()
            return jsonify({"error": "Invitation introuvable"}), 404

        cursor.execute("SELECT id FROM group_preferences WHERE invite_code = %s AND user_id = %s LIMIT 1", (invite_code, user_id))
        existing_row = cursor.fetchone()

        if existing_row:
            cursor.execute("""
                UPDATE group_preferences
                SET role=%s, hotel_type=%s, hotel_location=%s, activity_types=%s,
                    cafe_levels=%s, voyage_type=%s, budget=%s, hotel_name=%s,
                    cafe_name=%s, tranche_age=%s,
                    destination=%s, date_depart=%s, date_arrivee=%s, nuitees=%s
                WHERE id=%s
            """, (role, data.get("hotel_type",""), data.get("hotel_location",""),
                  ...
                  inv["destination"], inv["date_depart"], inv["date_arrivee"], inv["nuitees"],
                  existing_row['id']))
        else:
            cursor.execute("""
        ...
        conn.commit(); cursor.close(); conn.close()
        return jsonify({"status": "success", "message": "Préférences sauvegardées", "email_used": email}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        cursor.execute("""
            SELECT gp.role, gp.email, gp.hotel_type, gp.hotel_location,
                   ...
                   u.full_name, u.phone
            FROM group_preferences gp INNER JOIN users u ON u.id = gp.user_id
        ...
        def fmt(m):
            return {
                "role": m['role'], "email": m['email'], "full_name": m['full_name'],
                "phone": m['phone'], "hotel_type": m['hotel_type'],
                "hotel_location": m['hotel_location'], "activity_types": m['activity_types'],
                "cafe_levels": m['cafe_levels'], "voyage_type": m['voyage_type'],
                "budget": str(m['budget']) if m['budget'] else None,
                "hotel_name": m['hotel_name'], "cafe_name": m['cafe_name'],
                "tranche_age": m['tranche_age'], "destination": m['destination'],
    ...
    try:
        ...
        cursor.execute("""
            SELECT gp.role, gp.email, gp.hotel_type, gp.hotel_location,
                   ...
                   gp.destination, gp.date_depart, gp.date_arrivee, gp.nuitees,
                   u.full_name, u.phone
            FROM group_preferences gp LEFT JOIN users u ON u.id = gp.user_id
        ...
        def fmt_member(m):
            return {
                "role": m['role'], "email": m['email'], "full_name": m['full_name'],
                "phone": m.get('phone'), "hotel_type": m['hotel_type'],
                "hotel_location": m['hotel_location'], "activity_types": m['activity_types'],
                "cafe_levels": m['cafe_levels'], "voyage_type": m['voyage_type'],
                "budget": str(m['budget']) if m['budget'] else None,
                "hotel_name": m['hotel_name'], "cafe_name": m['cafe_name'],
...
def save_plan():
    ...
    for day in itinerary:
        ...
        line       = title
        if hotel_name: line += f' · 🏨 {hotel_name}'
        if activity:   line += f' · 🎯 {activity[:60]}'
        if line.strip(): highlights.append(line)
        ...
        transports.append({'day': title or f'Jour {len(transports)+1}', 'transport': transport_str})

        la = day.get('localActivity')
        if isinstance(la, dict):
            loisirs.append({'day': title, 'name': str(la.get('name','')), 'description': str(la.get('description','')), 'prix': str(la.get('prix',''))})
        elif isinstance(la, str) and la:
            loisirs.append({'day': title, 'name': la, 'description': '', 'prix': ''})
        else:
            loisirs.append({'day': title, 'name': '', 'description': '', 'prix': ''})
    ...
    try:
        ...
        cursor.execute('''
            INSERT INTO shared_plans
                (plan_code, leader_id, leader_email, plan_json, transport_json, loisir_json,
                 created_at, destination, date_debut, date_fin, nom)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            ON DUPLICATE KEY UPDATE
                leader_id=VALUES(leader_id), leader_email=VALUES(leader_email),
                plan_json=VALUES(plan_json), transport_json=VALUES(transport_json),
                loisir_json=VALUES(loisir_json), destination=VALUES(destination),
                date_debut=VALUES(date_debut), date_fin=VALUES(date_fin),
                nom=VALUES(nom), updated_at=NOW()
        ''', (plan_code, leader_id, leader_email,
              json.dumps(plan, ensure_ascii=False),
              json.dumps(transports, ensure_ascii=False),
              json.dumps(loisirs, ensure_ascii=False),
              datetime.utcnow().isoformat(), destination, date_debut, date_fin, nom))
        conn.commit()
    except Exception as e:
        if conn:
            try: conn.rollback()
    ...
    for email in guest_emails:
        try:
            highlights_html = ''.join(f'<li style="margin:4px 0;color:#374151">{escape(str(h))}</li>' for h in highlights[:5])
            html_body = f"""
<div style="font-family:Arial,sans-serif;max-width:540px;margin:auto;border-radius:16px;overflow:hidden;border:1px solid #E5E7EB;">
  <div style="background:linear-gradient(135deg,#042A66,#0A4DBF);padding:32px 24px;text-align:center">
    ...
    <p style="color:rgba(255,255,255,0.75);margin:10px 0 0;font-size:14px"><strong style="color:#fff">{name_h}</strong> vous partage un plan de voyage</p>
  </div>
  <div style="padding:28px 24px;background:#fff">
    <div style="background:#EEF4FF;border-radius:12px;padding:16px;margin-bottom:20px;">
      <table style="width:100%;border-collapse:collapse">
        <tr><td style="color:#7A90B4;font-size:13px;padding:4px 0">📍 Destination</td><td style="font-weight:700;text-align:right;color:#042A66;font-size:13px">{dest_h}</td></tr>
        <tr><td style="color:#7A90B4;font-size:13px;padding:4px 0">🛫 Départ</td><td style="font-weight:700;text-align:right;color:#042A66;font-size:13px">{ddebut_h}</td></tr>
        <tr><td style="color:#7A90B4;font-size:13px;padding:4px 0">🛬 Retour</td><td style="font-weight:700;text-align:right;color:#042A66;font-size:13px">{dfin_h}</td></tr>
      </table>
    </div>
    ...
    <p style="color:#9CA3AF;font-size:11px;text-align:center;margin-top:20px;">Ce message a été envoyé automatiquement par Pack&amp;Go</p>
  </div>
</div>"""
            send_email_brevo(
                to_email=email,
    ...
    return jsonify({
        'success': True, 'plan_code': plan_code, 'nom': nom,
        'emails_sent': results, 'total_guests': len(guest_emails), 'sent_count': sent_count,
    })
...
def plan_by_code():
    ...
    return jsonify({
        'valid': True, 'plan_code': row['plan_code'], 'nom': row['nom'],
        'destination': row['destination'], 'date_debut': row['date_debut'],
        'date_fin': row['date_fin'], 'leader_email': row['leader_email'],
        'created_at': row['created_at'], 'plan': plan
    })
...
def on_join_room(data):
    ...
    if not invite_code:
        emit("error", {"message": "inviteCode requis"}); return
...
def on_send_message(data):
    ...
    if not invite_code or not text: return
...
def group_ai_plan():
    ...
    hotels     = ai.filter_hotels(city_data["hotels"], resolved_prefs.get("hotel_type",""), resolved_prefs.get("hotel_location",""), resolved_prefs.get("hotel_name"))
    cafes      = ai.filter_cafes(city_data["cafes"], resolved_prefs.get("cafe_style",""), resolved_prefs.get("cafe_name"))
    ...
    try:
        ...
        return jsonify({
            "ville": city_data["original_name"], "days": days,
            "participants_count": len(participants), "resolved_prefs": resolved_prefs,
            "resolved_dates": resolved_dates, "gemini_error": str(exc), **fallback
        })
