...
/* ─────────────────────────────────────────────
   LOGIQUE CAFÉS — type déduit du prix
   Luxe      : prix max > 10 TND
   Normal    : 5 ≤ prix max ≤ 10 TND
   Coffee Shop: prix min ≥ 4 TND (max < 5)
───────────────────────────────────────────── */
...
export default function QuestionsScreen() {
  ...
  const inviteCode =
    (paramInviteCode && paramInviteCode.trim().toUpperCase()) ||
  ...
  // Cafés filtrés selon le type choisi par l'utilisateur
  ...
  const handleCafeLevelToggle = (v: string) => {
    toggleSelection(v, cafeLevels, setCafeLevels);
    setCafe("");
  };
  ...
  const handleSubmit = async () => {
    ...
    // ✅ CORRECTION : on n'envoie PAS d'email depuis le client pour le leader.
    // Le backend récupère l'email réel depuis la table users via user_id.
    // Cela garantit que c'est toujours l'email du compte connecté qui est enregistré,
    // jamais un email saisi manuellement dans le formulaire d'invitation.
    if (currentInviteCode) {
      ...
      try {
        ...
        if (result.email_used) {
          console.log("📧 Email leader enregistré:", result.email_used);
        }
      } catch (e) {
        console.warn("⚠️ save_group_preferences error:", e);
      }
    }
    ...
    try {
      ...
      console.error("Erreur save_preferences:", e);
    }
  };
      ...
      >
        ...
        <View style={styles.heroHeader}>
          ...
          <View style={styles.heroBadge}>
            <Text style={styles.heroBadgeText}>Personnalisation</Text>
          </View>
          <View style={styles.heroTitleRow}>
            ...
            />
            <View>
              <Text style={styles.heroTitle}>Votre séjour</Text>
              <Text style={styles.heroSubtitle}>
                Répondez pour personnaliser
              </Text>
            </View>
          </View>

          {inviteCode ? (
            <View style={styles.codeInHero}>
              <Text style={styles.codeInHeroLabel}>🔑 Code d'invitation</Text>
              <Text style={styles.codeInHeroValue}>{inviteCode}</Text>
            </View>
          ) : null}
        ...
        <View style={styles.card}>
          ...
          {cityHotels.length > 0 ? (
            <>
              <TouchableOpacity
                style={[
                  styles.hotelPickerBtn,
                  hotel ? styles.hotelPickerBtnSelected : null,
                ]}
                onPress={() => setHotelModalVisible(true)}
                activeOpacity={0.8}
              >
                <Text style={styles.hotelPickerBtnIcon}>🏨</Text>
                <View style={{ flex: 1 }}>
                  <Text
                    style={[
                      styles.hotelPickerBtnText,
                      hotel ? styles.hotelPickerBtnTextSelected : null,
                    ]}
                  >
                    {hotel
                      ? hotel
                  ...
                  {hotel && (
                    <Text style={styles.hotelPickerBtnSub}>
                      {"⭐".repeat(
                ...
                <Text style={styles.hotelPickerChevron}>
                  {hotel ? "✓" : "›"}
                </Text>
              </TouchableOpacity>
              {hotel && (
        ...
        <MultiOptionGroup
          ...
          onToggle={(v) => toggleSelection(v, activityTypes, setActivityTypes)}
          required
          ...
          {cityCafes.length > 0 ? (
            <>
              <TouchableOpacity
                style={[
                  styles.hotelPickerBtn,
                  cafe ? styles.hotelPickerBtnSelected : null,
                ]}
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  <Text
                    style={[
                      styles.hotelPickerBtnText,
                      cafe ? styles.hotelPickerBtnTextSelected : null,
                    ]}
                  >
                    {cafe
                      ? cafe
        ...
        >
          <View style={styles.submitInner}>
            <Text style={styles.submitText}>Enregistrer mon séjour</Text>
              ...
              {cityHotels.map((h) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.75}
                  >
                    <View style={styles.hotelItemLeft}>
                      <View
                        style={[
                          styles.hotelItemIcon,
                          isSelected && styles.hotelItemIconSelected,
                        ]}
                      >
                        <Text style={styles.hotelItemIconTxt}>🏨</Text>
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text
                          style={[
                            styles.hotelItemName,
                            isSelected && styles.hotelItemNameSelected,
                          ]}
                        >
                          {h.name}
                        </Text>
                        <Text style={styles.hotelItemStars}>
                          {"⭐".repeat(starCount)}
                        </Text>
                        <Text style={styles.hotelItemDesc}>
                          {h.description}
                        </Text>
                      </View>
                    </View>
                    {isSelected && (
                      <View style={styles.hotelItemCheck}>
                        <Text style={styles.hotelItemCheckTxt}>✓</Text>
                      </View>
                    )}
                  </TouchableOpacity>
                );
                  ...
                  >
                    <View style={styles.hotelItemLeft}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        {c.zone ? (
                          <Text style={styles.hotelItemDesc}>📍 {c.zone}</Text>
                        ) : null}

                        {c.prix ? (
                          <Text
                            style={[styles.hotelItemDesc, styles.cafePrixText]}
                          >
                            💰 {c.prix}
                          </Text>
                        ) : null}
...
const styles = StyleSheet.create({
  ...
  sectionLine: { flex: 1, height: 1, backgroundColor: "#C8D9EF" },
  sectionLabelText: {
    fontSize: 11,
    fontWeight: "700",
    color: BLUE_PRIMARY,
  ...
  chipIconSelected: { color: WHITE },
  chipText: { fontSize: 14, fontWeight: "600", color: BLUE_DEEP },
  ...
  pillText: { fontSize: 12, color: BLUE_PRIMARY, fontWeight: "600" },
  ...
  hotelPickerBtnIcon: { fontSize: 22 },
  hotelPickerBtnText: { fontSize: 14, fontWeight: "600", color: TEXT_MUTED },
  hotelPickerBtnTextSelected: { color: BLUE_DEEP, fontWeight: "700" },
  hotelPickerBtnSub: { fontSize: 12, color: TEXT_MUTED, marginTop: 2 },
  hotelPickerChevron: { fontSize: 20, color: BLUE_PRIMARY, fontWeight: "700" },
  ...
  hotelItemIconSelected: { backgroundColor: BLUE_PRIMARY },
  hotelItemIconTxt: { fontSize: 20 },
  hotelItemName: {
    fontSize: 14,
    fontWeight: "700",
    color: BLUE_DEEP,
  ...
  hotelItemNameSelected: { color: BLUE_PRIMARY },
  hotelItemStars: { fontSize: 12, marginBottom: 2 },
  hotelItemDesc: { fontSize: 11, color: TEXT_MUTED, lineHeight: 16 },
  hotelItemCheck: {
    width: 28,
    height: 28,
    borderRadius: 14,
  ...
  hotelItemCheckTxt: { color: WHITE, fontSize: 14, fontWeight: "800" },
  cafeTypeBadge: {
    alignSelf: "flex-start",
